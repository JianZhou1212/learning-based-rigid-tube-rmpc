...
for i = 1:length(N_sam_ini)
    figure(i)
    plot(W, 'wire', 1, 'edgecolor', c1, 'linewidth', 2.5);
    hold on
    plot(W_Hat_ini{i}, 'wire', 1, 'edgecolor', c2, 'linewidth', 2.5);
    hold on
    plot(W_true, 'wire', 1, 'edgecolor', c3, 'linewidth', 2.5);
    hold on
    plot(W_hat_opt, 'wire', 1, 'edgecolor', c4, 'linewidth', 2.5);
    hold on
    plot(Samples_ini{i}(1, :),Samples_ini{i}(2, :), 'color', c5, 'marker', 'x','markersize', 5);
    box on
    grid off
    xlim([-0.62, 0.62]);
    ylim([-0.25, 0.25]);
...
for i = 1:length(N_sam_ini)
    ...
    plot(F_N_True, 'wire', 1, 'edgecolor', c3, 'linewidth', 2.5);
    hold on
    plot(F_N_Hat, 'wire', 1, 'edgecolor', c2, 'linewidth', 2.5);
    hold on
    plot(F_N_RMPC, 'wire', 1, 'edgecolor', c1, 'linewidth', 2.5);
    hold on
    plot(F_N_Hat_Opt, 'wire', 1, 'edgecolor', c4, 'linewidth', 2.5);
    box on
    grid off
    xlim([-16, 16]);
    ylim([-8, 8]);
