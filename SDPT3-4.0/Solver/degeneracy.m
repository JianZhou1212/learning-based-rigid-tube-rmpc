...
%% for primal non-degeneracy, 
%%     we need the mx(r2+r*(n-r)) matrix [XB1 XB2] to have full row rank.
%% for dual non-degeneracy,   
%%     we need the mx((n-s)2) matrix ZB1 to have full column rank. 
%%*****************************************************************
%% SDPT3: version 4.0
%% Copyright (c) 1997 by
...
%%
    for p = 1:size(blk,1)
       ...
       elseif strcmp(blk{p,1},'u')
          error('ublk is not allowed at the moment.');         
       ...
       fprintf('\n    blkno = %1.0d, tol = %3.1e,%3.1e,  m = %2.0d',...
               p,tolx(p),tolz(p),m); 
       fprintf('\n    n= %2.0f,  r= %2.0f, s= %2.0f',n,r(p),s(p)); 
       fprintf('\n    n2-(n-r)2 = %2.0f',prim_rank(p));
       fprintf('\n    (n-s)2    = %2.0f',dual_rank(p));
       fprintf('\n    complemen = %2.1e  %2.1e\n',max(dx+dz),min(dx+dz));
       subplot(121) 
...
%%
    ...
    fprintf('    svd([XB1, XB2]): max, min = %2.1e %2.1e, cond = %2.1e\n',...
